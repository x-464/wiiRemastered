...
// fn all_isos_in_dir(dir: &Path) -> Result<Vec<PathBuf>, String> {
...
//     if isos.is_empty() {
//         return Err("No .iso files found in selected folder".to_string());
//     }
...
// #[tauri::command]
// pub fn unwrap_iso(app: AppHandle, iso_path: String) -> Result<Vec<String>, String> {
//     let iso = PathBuf::from(&iso_path);
//     if !iso.is_dir() {
//         return Err("Selected path is not a iso".to_string());
//     }

//     let isos = all_isos_in_dir(&iso)?;
//     let root = app_extract_root(&app)?;
//     let mut bnr_paths = Vec::new();

//     for iso_path in isos {
//         let iso_name = safe_stem(&iso_path);
//         let extract_dir = root.join(&iso_name);

//         run_wit_extract(&iso_path, &extract_dir)?;

//         // let bnr_path = extract_dir.join("DATA").join("files").join("opening.bnr");
//         let bnr_path = find_opening_bnr(&extract_dir)
//             .ok_or_else(|| format!("opening.bnr not found under {}", extract_dir.display()))?;

//         if !bnr_path.exists() {
//             return Err(format!(
//                 "opening.bnr not found for extracted ISO: {}",
//                 bnr_path.display()
//             ));
//         }

//         bnr_paths.push(bnr_path.display().to_string());
//     }
